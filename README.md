...
> Answer:
  ...
  I think that the main benefit of using client side routing is that you get faster page transitions due to the entire site already being downloaded. This may cause longer initial load times, but after that they are much faster because there are no more http requests going out. With client-side routing we only load the content we need as opposed to reloading the entire page every time we need to update a view.
